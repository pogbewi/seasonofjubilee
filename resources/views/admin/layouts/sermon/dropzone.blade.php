...
<div id="{{$dropzoneId}}" class="dropzone">
    <div class="dz-default dz-message">
        ...
        <p class="text-muted">{{ $desc or 'Any related files you can upload' }} <br>
            <small>One file can be max {{ config('savysoft.attachment.max_size', 0) / 5000 }} MB</small></p>
    </div>
</div>
<!-- Dropzone {{ $dropzoneId }} -->
@push('scripts')
...
<script>
    ...
    $(function () {
        ...
        $("#{{ $dropzoneId }}").dropzone({
            url: "{{ route('admin.sermon.add.media', $sermon->slug) }}",
            addRemoveLinks: true,
            maxFilesize: {{ isset($maxFileSize) ? $maxFileSize : config('savysoft.attachment.max_size', 5000) / 1000 }},
            acceptedFiles: "{!! isset($acceptedFiles) ? $acceptedFiles : config('savysoft.attachment.allowed') !!}",
    ...
    function getIconFromFilename(file) {
        ...
        if( file.type.indexOf('image') === -1 ) {
            ...
            return "/storage/frontend/img/"+ext+".png";
        }

        // return a placeholder for other files
        return '/storage/frontend/img/txt.svg';
    }
</script>
@endpush